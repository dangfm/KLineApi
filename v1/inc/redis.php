...
class FMRedisCluster{
    ...
    public function connect($config=array('host'=>'127.0.0.1','port'=>6379,'auth'=>''), $isMaster=true){
        ...
        //echo $config["host"].":".$config["port"]." "+$config["auth"];
        // 设置 Master 连接
        if($isMaster){
            $this->_linkHandle['master'] = new Redis();
            $ret = $this->_linkHandle['master']->connect($config['host'],$config['port'],30);
            if ($config['auth']!='') {
            ...
            $ret = $this->_linkHandle['slave'][$this->_sn]->connect($config['host'],$config['port'],30);
            if ($config['auth']!='') {
                $this->_linkHandle['slave']->auth($config['auth']);
            }
            ++$this->_sn;
        }
        return $ret;
...
$redis = new FMRedisCluster();
$redis->connect(array('host'=>$config->redis->host,'port'=>$config->redis->port,'auth'=>$config->redis->auth));
